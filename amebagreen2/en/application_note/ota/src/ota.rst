...
.. only:: RTL8721D
   ...
   The application image named :file:`km0_km4_app.bin`, including KM0, KM4 non-secure application image and KM4 secure image, can be updated through OTA, 
   which can be chosen to boot from OTA1 or OTA2. The layout of the whole application image is illustrated below.

.. only:: RTL8711D

   The application image named :file:`km0_km4_app.bin` can be updated through OTA, which can be chosen to boot from OTA1 or OTA2.
   The layout of the whole application image is illustrated below.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
1. Modify the version number in configuration file :file:`manifest.json` under ``{SDK}\amebadplus_gcc_project``.
...
2. Change the bootloader version of anti-rollback and enable anti-rollback if necessary.
   ...
   .. note::
      ...
      - If bootloader and application do not use the same anti-rollback version, modify :func:`BOOT_OTA_GetCertRollbackVer()` in ``{SDK}\component\soc\amebadplus\bootloader\boot_ota_km4.c`` and define another anti-rollback version in OTP for the application.
...
.. table:: Firmware header
   ...
   +---------------+----------------+---------+-------------------------------------------------------------------+
   | Header Number | 0x04           | 4 bytes | Number of OTA Header. For |CHIP_NAME|, this value can be 1, 2.    |
   +---------------+----------------+---------+-------------------------------------------------------------------+
   | Signature     | 0x08           | 4 bytes | OTA signature is string. For |CHIP_NAME|, this value is ``OTA``.  |
   +---------------+----------------+---------+-------------------------------------------------------------------+
   | Header Length | 0x0C           | 4 bytes | Length of OTA header. For |CHIP_NAME|, this value is 0x18.        |
   +---------------+----------------+---------+-------------------------------------------------------------------+
