...
Overview
----------------
Media is a whole media architecture aiming to provide media interfaces for applications to use. It provides a player named RTPlayer, which can be used to control the playback of an audio file.

The RTPlayer supports the following kinds of audio sources:
...
Architecture
---------------
Applications interact with media according to the following architecture.
...
Callbacks
~~~~~~~~~~~
The Media provides a registration function ``RTPlayer_SetCallback(struct RTPlayer *player, struct RTPlayerCall back *callbacks)`` for applications to monitor state change and runtime errors during playback or streaming. Applications need to implement the interfaces defined in struct ``RTPlayerCallback``.

Relevant interfaces are described as bellow:

- ``void (*OnRTPlayerStateChanged)(const struct RTPlayerCallback *listener, const struct RTPlayer *player, int state);``

  Notify monitors the player status is changed. The parameter state is one of ``RTPlayerStates``. ``RTPlayerStates`` is shown as bellow:
...
- ``void (*OnRTPlayerInfo)(const struct RTPlayerCallback *listener, const struct RTPlayer *player, int info, int extra);``
  ...
  Notify monitors some player information. The parameter info indicates the information type, see ``RTPlayerInfos``. The parameter extra indicates the information code, for example, the value of extra is the percentage of buffered data during a buffering state. ``RTPlayerInfos`` is shown as bellow:
...
- ``void (*OnRTPlayerError)(const struct RTPlayerCallback *listener, const struct RTPlayer *player, int error, int extra);``
  ...
  Notify monitors some player error occurs. The parameter error indicates the error type. For details, see ``RTPlayerErrors``. ``RTPlayerErrors`` is shown as bellow:
...
1. Relevant interfaces are described as bellow:
...
The RTPlayer sets a starting water level for http streaming source, playback will not start until enough audio data is buffered. Application calls ``RTPlayer_Prepare(…)`` or ``RTPlayer_PrepareAsync(…)`` to prepare the streaming source.

- ``RTPlayer_Prepare(…)`` is a synchronous function, application can call ``RTPlayer_Start(…)`` after it.
- ``RTPlayer_PrepareAsync(…)`` is an asynchronous function, application should not call ``RTPlayer_Start(…)`` until receiving ``RTPlayerCallback``. ``OnRTPlayerStateChanged(…, …, RTPLAYER_PREPARED)``.

For http source, the RTPlayer will continuously check the downloaded data and update the downloaded percentage to monitors via ``RTPlayerCallback``. ``OnRTPlayerInfo(…, …, RTPLAYER_INFO_BUFFERING_INFO_UPDATE)``. When the downloaded data is enough, the RTPlayer will trigger ``RTPlayerCallback``. ``OnRTPlayerStateChanged (…, …, RTPLAYER_PREPARED)`` and application can start the playback. The detailed principle is shown as in figure below.

.. figure:: ../figures/httpsource_stream_buffering_principle_during_starting.svg
   :scale: 110%
   :align: center

   HttpSource stream buffering principle during starting

During playback, if the remaining downloaded data is not enough, RTPlayer will trigger ``RTPlayerCallback.OnRTPlayerInfo(…, …, RTPLAYER_INFO_BUFFERING_START)``. Application can update users' display interfaces to indicate that the current network status is poor. During this time, the RTPlayer will continuously check the downloaded data and update the downloaded percentage to monitors via ``RTPlayerCallback.OnRTPlayerInfo(…, …, RTPLAYER_INFO_BUFFERING_INFO_UPDATE)``. When the downloaded data is enough, the RTPlayer will trigger ``RTPlayerCallback.OnRTPlayerInfo(…, …, RTPLAYER_INFO_BUFFERING_END)`` and application can resume the playback. The detailed principle is shown as below.

.. figure:: ../figures/httpsource_stream_buffering_principle_during_playing.svg
   :scale: 110%
   :align: center
...
After :func:`RTPlayer_SetSource`, you need to prepare the RTPlayer by call ``RTPlayer_Prepare()`` or ``RTPlayer_PrepareAsync()``.

- ``RPlayer_Prepare(…)`` is a synchronous function, you can call ``RTPlayer_Start(…)`` immediately after it.
...
- ``RTPlayer_PrepareAsync(…)`` is an asynchronous function, you should not call ``RTPlayer_Start(…)`` until receiving ``RTPlayerCallback``. ``OnRTPlayerStateChanged(…, …, RTPLAYER_PREPARED)``.
...
When the RTPlayer is done preparing, it enters the Prepared state, which means you can call ``RTPlayer_Start()`` to make it play the media. At that point, you can move between the Started, Paused and PlaybackCompleted states by calling such methods as ``RTPlayer_Start()`` and ``RTPlayer_Pause()``, amongst others. When you call ``RTPlayer_Stop()``, however, notice that you cannot call ``RTPlayer_Start()`` again until you prepare the RTPlayer again.
...
An RTPlayer can consume valuable system resources. Therefore, you should always take extra precautions to make sure you are not hanging on to an RTPlayer instance longer than necessary. When the playback is done, you should always call ``RTPlayer_Stop()`` and ``RTPlayer_Reset()`` to release resources and restore the state. After calling ``RTPlayer_Reset(),`` you will have to initialize it again by setting the source and calling ``RTPlayer_Prepare()``.

- Call ``RTPlayer_Stop()`` when playback complete by receiving ``RTPLAYER_PLAYBACK_COMPLETE`` event.

  .. code-block:: c
  
     RTPlayer_Stop(player);

- Call ``RTPlayer_Reset()`` when playback stopped by receiving ``RTPLAYER_STOPPED`` event.
