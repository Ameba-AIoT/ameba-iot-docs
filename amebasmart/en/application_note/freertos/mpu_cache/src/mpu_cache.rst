...
-------------------------
The Memory Protection Unit (MPU) is a component provided by Arm and is used to provide hardware protection by software definition. The code in SDK provides the :class:`mpu_region_config` struct to set the region memory attribute of MPU.
...
.. table::
   ...
   +--------------+---------------------------------------------------------------------------+
   | Parameters   | - :literal:`attr_idx`: region memory attribute index, which can be 0 ~ 7. |
   |              |                                                                           |
   |              | - :literal:`mem_attr`: region memory attributes.                          |
   +--------------+---------------------------------------------------------------------------+
   | Return       | None                                                                      |
   +--------------+---------------------------------------------------------------------------+
...
----------------------------------------------------------------------------------
Add :literal:`SRAM_NOCACHE_DATA_SECTION` before the buffer definition to define a data buffer with non-cacheable attribute.
...
Cache Consistency When Use DMA
------------------------------------------------------------
...
.. caution::
   The DMA operation address requires exclusive ownership of a complete cache line. You can define the buffer using :literal:`malloc()` or :literal:`ALIGNMTO(CACHE_LINE_SIZE) u8 op_buffer[CACHE_LINE_ALIGMENT(op_buffer_size)]`.
...
5. DCache_Invalidate (this step is mandatory)
   ...
   .. caution::
      ...
      - For CPUs with Automatic data prefetching and monitoring capabilities, such as Cortex-A32/DSP, e.g., Cortex-A32 reads the contents of adjacent addresses of the Rx buffer, Cortex-A32 starts linefills in the background to bring the old values of the Rx buffer back into the cache.
...
6. CPU read Rx buffer (the value returned by DMA RX).
   ...
   .. caution::
      DCache_Clean/DCache_CleanInvalidate operations write entire cache lines to memory.
