.. _amebasmart_virtual_file_system:
...
.. figure:: ../figures/architecture_of_vfs.svg
   :scale: 130%
   :align: center
...
VFS Initialization
------------------------------------
By default, the VFS has been initialized in :file:`main.c` as follows:
...
The :func:`vfs_user_register()` API will mount VFS to the Flash (or SD card) by users' configuration. If failed to mount, this API will check whether the Flash is clean (0xFF). And if the Flash is clean, it will program the Flash to initialize VFS.
...
:**region**: Flash partition (VFS1 or VFS2, described in :ref:`vfs_on_flash`)
...
VFS on SD card
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The SD card is only supported by FatFS, `vfs_type` should be set to FatFS when `interface` is set to SD card. Menuconfig VFS Fatfs SD configuration should be set.

.. figure:: ../figures/sd_conf1.png
   :scale: 90%
   :align: center

.. figure:: ../figures/sd_conf2.png
   :scale: 90%
   :align: center

Hardware Setup
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Connect your EVB board to PC with micro-USB cable, and then plug in a compatible SD card to the card connector, as shown in the following figure.

.. figure:: ../figures/hardware_setup1.jpeg
   :scale: 80%
   :align: center

.. figure:: ../figures/hardware_setup2.jpeg
   :scale: 80%
   :align: center
...
Software Setup
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Change `vfs_type` and `interface` parameters of :func:`vfs_user_register()` to `VFS_FATFS` and `VFS_INF_SD` in :file:`main.c`.
...
(1)    Prepare the read-only files and convert them into a FAT-formatted bin file. Refer to Section :ref:`fatfs_bin_file_generation` for the method.

(2)    Name the bin file :file:`fatfs.bin` and place it in ``{SDK}\amebasmart_gcc_project`` directory.

(3)    Enable the following configurations in the menuconfig:

   .. figure:: ../figures/config_within_app1.png
      :scale: 90%
      :align: center

   .. figure:: ../figures/config_within_app2.png
      :scale: 90%
      :align: center

(4)    Rebuild the application firmware. The application firmware (:file:`km0_km4_ca32_app.bin` or :file:`km0_km4_app.bin`) will include a read-only VFS area in FAT format, which will be mounted during the startup process.
...
(6)    For the file usage method, refer to Section :ref:`common_file_operation` or :ref:`key_value_operation`.

.. note::
   To optimize the effective space utilization of the read-only file system, only FAT format is currently supported.

.. _common_file_operation:
...
1. Prepare a needed object folder including files before generating LittleFS bin files. For example:
   ...
   .. figure:: ../figures/little_bin_file_generation_step1.png
      :scale: 90%
      :align: center
...
2. Use the command ``$./mklittlefs -b 4096 -p 256 -s 0x20000-c test image_littlefs.bin`` in ``mklittlefs`` tool located at ``\tools\littlefs`` to generate LittleFS bin files.
   ...
   .. figure:: ../figures/little_bin_file_generation_step2.png
      :scale: 90%
      :align: center
...
3. Download the image to the Flash.
   ...
   The start address of image should be VFS Flash region start address mentioned in Section :ref:`vfs_on_flash`. Test logs are shown below:
...
4. Use command ``root@ubuntu # sudo cp hello.txt ./fs`` to copy the files that users want to store into :file:`test.bin`.

   In this step, ``hello.txt`` is stored in :file:`test.bin`.

5. Use command ``root@ubuntu # sudo umount ./fs`` to generate the FatFS file after unmounting :file:`test.bin`.

Users should find other related information about this topic from the internet, and copy test.bin into user data area of Flash finally.
