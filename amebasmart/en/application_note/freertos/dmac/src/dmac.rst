...
.. table::
   ...
   +-------+--------+----------------------------+----------------------------+
   | Slave | Clock  | Writing 64 bytes           | Reading 64 bytes           |
   +=======+========+============================+============================+
   | SRAM  | 300MHz | (64*8)/(500ns)=1024Mbps    | (64*8)/(340ns)=1505.88Mbps |
   +-------+--------+----------------------------+----------------------------+
   | PSRAM | 250MHz | (64*8)/(270ns)=1896.30Mbps | (64*8)/(400ns)=1280Mbps    |
   +-------+--------+----------------------------+----------------------------+
   | DDR   | 530MHz | (64*8)/(250ns)=2048Mbps    | (64*8)/(310ns)=1651.61Mbps |
   +-------+--------+----------------------------+----------------------------+
   | Audio | 40MHz  | (64*8)/(1200ns)=426.67Mbps | (64*8)/(410ns)=1248.78Mbps |
   +-------+--------+----------------------------+----------------------------+
   | SPI   | 75MHz  | (64*8)/(510ns)=1003.92Mbps | (64*8)/(700ns)=731.43Mbps  |
   +-------+--------+----------------------------+----------------------------+
...
Data Size
^^^^^^^^^^^^^^^^^^
The following figure illustrates the setting of GDMA transmission data size. The block_ts indicates the amount of data that will be transferred in a single data block. It needs to be set to the total number of data/SRC_TR_WIDTH, and the maximum value is ``0xFFFF``.
...
.. table::
   ...
   +-----------------------------------+--------------------------+------------------------+
   | TT_FC[2:0] field of CTLx register | Direction                | Flow controller        |
   +===================================+==========================+========================+
...
.. note::
      - When reading from and writing to peripherals, the SRC_TR_WIDTH/DST_TR_WIDTH is completely determined by the width of the peripherals.

      - When reading from and writing to memory:
...
In auto-reload mode, the source and destination can independently select which method to use.
...
.. table::
   ...
   +--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Multi-block DMA transfer types | Setting                                | Introduction                                                                                                                                                                                   |
   +================================+========================================+================================================================================================================================================================================================+
   | Src auto-reload                | PGDMA_InitTypeDef->GDMA_ReloadSrc = 1; | For multi-block transfers, the SAR register can be auto-reloaded from the initial value at the end of each block, and DST address is contiguous, as shown in :ref:`mbd_source_auto_dest_cont`. |
   |                                |                                        |                                                                                                                                                                                                |
   |                                | PGDMA_InitTypeDef->GDMA_ReloadDst = 0; |                                                                                                                                                                                                |
   +--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Dst auto-reload                | PGDMA_InitTypeDef->GDMA_ReloadSrc = 0; | For multi block transfers, the DAR register can be auto-reloaded from its initial value at the end of each block, and the SRC address is contiguous.                                           |
   |                                |                                        |                                                                                                                                                                                                |
   |                                | PGDMA_InitTypeDef->GDMA_ReloadDst = 1; |                                                                                                                                                                                                |
   +--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Src & Dst auto-reload          | PGDMA_InitTypeDef->GDMA_ReloadSrc = 1; | For multi block transfers, the SAR and DAR register can be auto-reloaded from its initial value at the end of each block, as shown in :ref:`mbd_source_dest_auto`.                             |
   |                                |                                        |                                                                                                                                                                                                |
   |                                | PGDMA_InitTypeDef->GDMA_ReloadDst = 1; |                                                                                                                                                                                                |
   +--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
.. table::
   ...
   +--------------------------+-----------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Src: Auto-reloading      | PGDMA_InitTypeDef->GDMA_ReloadSrc = 1;  | In source, SAR register can be auto-reloaded from the initial value at the end of each block, as shown in :ref:`mbd_source_auto_link_dest`.               |
   |                          |                                         |                                                                                                                                                           |
   | Dst: Link list           | PGDMA_InitTypeDef->GDMA_SrcAddr = pSrc; |                                                                                                                                                           |
   |                          |                                         |                                                                                                                                                           |
   |                          | PGDMA_InitTypeDef->GDMA_LlpDstEn = 1;   |                                                                                                                                                           |
   +--------------------------+-----------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Src: Link list           | PGDMA_InitTypeDef->GDMA_LlpSrcEn = 1;   | Source memory is organized in the form of a linked list, and destination memory is a continuous data block, as shown in :ref:`mbd_link_source_cont_dest`. |
   |                          |                                         |                                                                                                                                                           |
   | Dst: Continue address    | PGDMA_InitTypeDef->GDMA_DstAddr = pDst; |                                                                                                                                                           |
   +--------------------------+-----------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   |                          | PGDMA_InitTypeDef->GDMA_ReloadDst = 1;  |                                                                                                                                                           |
   +--------------------------+-----------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------+
   | Src: Link list           | PGDMA_InitTypeDef->GDMA_LlpSrcEn = 1;   | Both source and destination data blocks are organized in linked lists, and show in :ref:`mbd_link_source_dest`.                                           |
   |                          |                                         |                                                                                                                                                           |
   | Dst: Link list           | PGDMA_InitTypeDef->GDMA_LlpDstEn = 1;   |                                                                                                                                                           |
   +--------------------------+-----------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------+
...
*************************
There are two modes: Increment and no change.

Increment: indicates whether to increment the source address on every source transfer. If the device is fetching data from a source peripheral FIFO with a fixed address, then set this field to "No change". Incrementing is done for alignment to the next `CTLx.SRC_TR_WIDTH` boundary.

Destination Address Increment
******************************
There are two modes: Increment and no change.

Increment: indicates whether to increment destination address on every destination transfer. If the device is writing data to a destination peripheral FIFO with a fixed address, then set this field to "No change". Incrementing is done for alignment to the next `CTLx.DST_TR_WIDTH` boundary.
...
.. note::
   In the following situation, channels are inactive:

   - `CFGx.INACTIVE` can only be activated after Memory has been written, and then canceled.
   - The data of peripheral is 4bytes, but the FIFO of DMAC is only 2bytes. There is no writing at this time and CFGx.INACTIVE is activated directly.
...
.. table::
   ...
   |           |                                                                                                                                                                                                                                                                       |
   |           | 5. Do :func:`DCache_Invalidate()` in Rx Done Handler to clean the old data, to avoid the problem that the data in the cache is inconsistent with the data in the memory after RX done if the CPU read or write allocate the DST buffer during GDMA transmission.      |
   |           |                                                                                                                                                                                                                                                                       |
   |           |                                                                                                                                                                                                                                                                       |
   |           |    .. note::                                                                                                                                                                                                                                                          |
...
Aligning the buffer address with the cache line will reduce the problem of inconsistent cache and memory data, and details can be found in section :ref:`cache_consistency_using_dma`.
