...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   OTP physical map can store six keys for Crypto to use as secret keys, which can only be accessed by crypto engine and will not be tampered with or read by attackers.
   The premise is that the secret key needs to be programmed into OTP physical map.

   .. tabs ::
      .. include:: 21Dx_crypto_engine_otpkeys.rst
      .. include:: 26E20E_crypto_engine_otpkeys.rst
      .. include:: 30E_crypto_engine_otpkeys.rst
...
If a hash algorithm is selected, and the message length doesn’t exceed 245745 (=15 * (214 -1)) bytes, Hash APIs ``rtl_crypto_xxx`` can be used to calculate the digest.

If the message length exceeds 245745 bytes or you want to divide this message into many particular size blocks to process them, you could use ``Sequential Hash Mechanism`` to verify the hash function.
...
.. note::
      ...
      - After Application Core(like KM4 or HP) power gating, IPsec needs to be initialized again to work normally.
