...
.. table:: Download interface supported by different Ameba SoCs
   :width: 100%
   :widths: auto

   +------------+------------+-------------------------+
   | Ameba SoC  | Flash type | Download interface      |
   |            |            +--------------+----------+
   |            |            | UART         | USB      |
   +============+============+==============+==========+
   | RTL8730E   | NOR Flash  | √            | √        |
...
.. only:: FreeRTOS
   ...
   .. table::
      ...
      +---------------------------+----------------------+---------------------------+--------------+
      | Ameba SoC                 | Image name           | Description               | Mandatory?   |
      +===========================+======================+===========================+==============+
      | RTL8730E                  | km4_boot_all.bin     | KM4 bootloader            | √            |
      |                           +----------------------+---------------------------+--------------+
      |                           | km0_km4_ca32_app.bin | KM0/KM4/CA32 applications | √            |
      +---------------------------+----------------------+---------------------------+--------------+
      | | RTL8726EA               | km4_boot_all.bin     | KM4 bootloader            | √            |
      | | RTL8713EC               +----------------------+---------------------------+--------------+
      | | RTL8720EA               | kr4_km4_app.bin      | KR4/KM4 applications      | √            |
      | | RTL8710EC               |                      |                           |              |
      +---------------------------+----------------------+---------------------------+--------------+
      | RTL8726EA/RTL8713EC       | dsp_all.bin          | DSP image                 | √            |
      +---------------------------+----------------------+---------------------------+--------------+
      | RTL8720EA/RTL8710EC       | dsp_all.bin          | DSP image                 | X            |
      +---------------------------+----------------------+---------------------------+--------------+
      | RTL8721Dx/RTL8711Dx       | km4_boot_all.bin     | KM4 bootloader            | √            |
      |                           +----------------------+---------------------------+--------------+
...
.. only:: FreeRTOS
   ...
   4. Click the :menuselection:`Browse` button to select the images to be programmed.
      ...
      .. note::
         ...
         However, to formally change the Flash layout, it is suggested to use :mod:`Device Profile Editor` other than :mod:`Image Tool` and the Flash layout in SDK shall bechanged accordingly.
