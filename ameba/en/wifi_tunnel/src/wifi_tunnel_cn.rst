...
.. figure:: ../../../en/wifi_tunnel/figures/wifi_tunnel_topology_capacity.svg
   :scale: 140%
   :align: center
...
- 应用层软件开发无感:
  ...
  - 所有的mesh协议都在WiFi驱动层实现，不管是root节点还是子节点，应用层都可以视为当前的节点是一个和AP连接的WiFi Station。
  - WiFi配网程序无需更新。

- 百微妙级别的快速配对和切换:

  - 当检测到信号更好的父节点的时候，子节点可以快速的从旧的父节点切换到新的父节点，而不影响数据通信。
  - 当父节点发生问题的时候(掉电或者挂住)，子节点可以迅速检测到并切换到另外一个父节点而不影响数据通信。
  - 一个节点可以携带其所有的子节点一起切换到另外一个父节点，所有节点的数据通信不受影响。

- 经过数跳的设备也具备较高的吞吐量:

  - R-Mesh 转发无需经过 TCPIP 协议栈， 数据转发都在底层驱动实现，可以节省 SRAM 和 MCU 算力。
  - 软件处理时间极短，可以获得更好的吞吐量。
...
Wi-Fi R-Mesh 的数据流
--------------------------------------------
Wi-Fi R-Mesh 直接在WiFi 驱动层实现数据转发, 消耗极少的SRAM和MCU算力。

由于需要极少的软件处理过程，所以即使经过几跳的节点也会有很好的吞吐量。

.. figure:: ../../../en/wifi_tunnel/figures/wifi_tunnel_data_flow.svg
   :scale: 130%
   :align: center
...
Wi-Fi R-Mesh 的网络容量
------------------------------------------
Wi-Fi R-Mesh 中每个root下可以连接的子节点个数称为R-Mesh 的网络容量。

如下图所示的例子中，网络容量为 4，则每个root只可以连接4个节点，不管拓扑的形式是哪一种。

- 拓扑 0: 所有子节点都直接连接到root。
- 拓扑 3: 4个节点形成一个4跳的线型网络
- 也可能是拓扑 0和拓扑 3之间的其它拓扑结构
...
Wi-Fi R-Mesh NAT
------------------------------------------
由于R-Mesh的每个节点都会和AP创建真实的WIFI连线，在AP 可以连接的Station数量受限的情况下，R-Mesh可以支持的节点个数也会受限。
基于此，我们可以使用station+softap基于NAT协议来扩展R-Mesh中节点的数量。此时，root节点及子节点会和softap进行连线，
而不是和AP进行连线. NAT协议用于AP网络和R-Mesh网络之间的数据转发.

在R-Mesh中我们成这样的节点为R-NAT节点.

如下图所示，R-NAT节点放在root和AP之间，用于扩展可以接入网络的节点的数量。

.. figure:: ../../../en/wifi_tunnel/figures/wifi_tunnel_nat.svg
   :scale: 140%
   :align: center
...
通过Iperf测试吞吐量
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
与普通的Station或者AP模式无异，我们可以通过iperf测试R-Mesh的吞吐量。如下图所示，可以在AP端和节点端的PC上输入iperf指令即可。

.. figure:: ../../../en/wifi_tunnel/figures/tp_test_ipref.svg
   :scale: 140%
   :align: center
...
.. figure:: ../../../en/wifi_tunnel/figures/rmesh_rtt.png
   :scale: 80%
   :align: center
...
.. figure:: ../../../en/wifi_tunnel/figures/rmesh_demo_tool_connection.svg
   :scale: 135%
   :align: center
...
.. figure:: ../../../en/wifi_tunnel/figures/rmesh_demo_tool.png
   :scale: 50%
   :align: center
...
测试步骤如下
^^^^^^^^^^^^
- 将AP和PC通过网络连接，然后执行gravitation.exe

- 打开gravitation文件夹下的文件 :file:`config.yaml`, 配置所要测试的AP，你可以同时添加多个AP：
...
联系 <claire_wang@realsil.com.cn>索取R-Mesh wlan lib，然后通过如下SDK编译image即可使用R-Mesh：

`IoT SDK <https://github.com/Ameba-AIoT/ameba-rtos>`_
