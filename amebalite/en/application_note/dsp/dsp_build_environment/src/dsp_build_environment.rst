...
.. table:: 
   ...
   +==============================+==========+===============================================+===========================================+
   | HiFi5 DSP SDK tools and docs |                                                          | Contact Realtek FAE for help              |
   +------------------------------+----------+-----------------------------------------------+-------------------------------------------+
   | Xtensa Xplorer installer     | Linux    | - Xplorer-9.0.18-linux-x64-installer.bin      |                                           |
   |                              |          |                                               |                                           |
   |                              |          | - XtensaTools_RI_2021_8_patch734507_linux.tgz |                                           |
   +------------------------------+----------+-----------------------------------------------+-------------------------------------------+
   | Xtensa Xplorer installer     | Windows  | - Xplorer-9.0.18-windows-installer.exe        |                                           |
   +------------------------------+----------+-----------------------------------------------+-------------------------------------------+
   | HiFi5 DSP configurations     | Linux    | - HIFI5_PROD_1123_asic_UPG_linux_redist.tgz   |                                           |
   |                              |          |                                               |                                           |
   |                              |          | - HIFI5_PROD_1123_asic_wUPG_linux_redist.tgz  |                                           |
   +------------------------------+----------+-----------------------------------------------+-------------------------------------------+
   | HiFi5 DSP configurations     | Windows  | - HIFI5_PROD_1123_asic_UPG_win32_redist.tgz   | Get these packages from Realtek released  |
   |                              |          |                                               |                                           |
   |                              |          | - HIFI5_PROD_1123_asic_wUPG_win32_redist.tgz  | DSP SDK ("configurations" folder)         |
   +------------------------------+----------+-----------------------------------------------+-------------------------------------------+
   | Xtensa Xplorer license       |                                                          | Xtensa Xplorer license is essential.      |
   +------------------------------+----------+-----------------------------------------------+-------------------------------------------+
...
.. note::
      - If you have not purchased Xtensa Xplorer license before, contact Cadence or Realtek FAE.
...
Consult a Cadence's staff if there are some the Xtensa Xplorer installer or license issues. If you do not know how to contact a Cadence's staff, contact Realtek FAE first.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The Xtensa on Chip Debugger Daemon (xt-ocd) is a powerful GDB-based debugging tool. It is not installed by default with the Xplorer IDE. A self-extracting executable installer is included with the Xplorer. The following is a brief installation method of xt-ocd. For more xt-ocd installation guide, refer to :file:`xtensa_debug_guide.pdf`.

- Windows: If Xplorer's installation path is :file:`C:\usr\xtensa\XtDevTools\downloads\RI-2021.8\tools\xt-ocd-14.08-windows64-installer`, due to differences in installation paths and software versions, the paths and installer's name may be different.

- Linux: Assuming Xplorer is installed under the ``/opt/xtensa`` path:
...
Installing Toolchains on Linux Server
--------------------------------------------------------------------------
...
If Xplorer GUI cannot be opened through X11, you can manually download and install the patch. The patch we provide (:file:`XtensaTools_RI_2021_8_patch734507_linux.tgz`) is not necessarily the latest, so it is recommended that users consult Cadence or download the latest patch for XtensaTools_RI_2021_8 through XPG. First copy the patch package to the xtensa tool path (for example: ``/opt/xtensa/XtDevTools/install/tools/``), then decompress the patch and overwrite the original RI-2021.8-linux folder.
...
2. Example directory structure after decompression:

.. figure:: ../figures/directory_structure_after_decompression.jpeg
   :scale: 90%
   :align: center

3. Install configuration (for window ABI):
...
4. Input Xtensa Tools Path (change according to the actual installation path):
...
Then, you can use the same installation procedure to install the CALL0 ABI configuration package: :file:`HIFI5_PROD_1123_asic_UPG_linux_redist.tgz`
...
Compiling with CMD
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
You can use auto_build.sh to compile project:
...
Xplorer will copy project_dsp to a new workspace *auto_ws* then build the project. The generated dsp_all.bin and dsp.bin are saved in the default ``<SDK>/dsp/project/image`` directory. Therefore, some intermediate compilation files will remain in the *auto_ws* directory. Refer to :file:`<SDK>/dsp/project/auto_build/readme.txt` for the use of :file:`auto_build.sh`.

Troubleshooting
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
- The python script version for post-processing is recommended to be python3.8. You may encounter problems if you use python3.11. If you do not want to change the default Python version of the system, refer to the note in Section *Build a Program* to change the post-processing command (right click on :guilabel:`project_dsp`, then: :menuselection:`Properties -> builder -> internal -> Post-build Step`). Specify the python3.8 in the post-build command.
...
- Add a file to the virtual folder:
...
.. note::
   Modify project configuration (e.g. modify compilation options, add and delete files, etc.), a GUI Xplorer IDE is required. On linux server, the Xplorer GUI window can be displayed on the local terminal using the X11 protocol. The method is as follows: Check the contents of file :file:'/etc/ssh/sshd_config'. In this file flag ``X11Forwarding`` should be set to yes. Then:
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
On a Linux server, you can run auto_build.sh to compile the default project.
...
Image path: :file:`project/image/dsp_all.bin`

The configuration file for the automatic compilation script is: :file:`project/auto_build/dsp_batch.xml`. In this script, you can modify the project configuration (HIFI5_PROD_1123_asic_wUPG or HIFI5_PROD_1123_asic_UPG).



.. note::
      - Add the Xtensa Tools directory to your system PATH. In our case, it is ``/opt/xtensa/XtDevTools/install/tools/RI-2021.8-linux/XtensaTools/bin``
...
You will find a *HelloWorld* project added to the Project Explorer automatically after going over all steps in the previous chapters. Now let's add *project_dsp* included in the SDK into the workspace.

2. Click :menuselection:`File > Import`

3. Click :menuselection:`General > Existing Projects into Workspace > Next`

.. figure:: ../figures/import_existing_projects_into_workspace.svg
   :scale: 90%
   :align: center

4. Set contents in the read box as above, and click :guilabel:`Finish`.
...
Building Project
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
In the upper part of Xtensa Xplorer, set *P*: to "project_dsp", *C*: to "HIFI5_PROD_1123_asic_UPG" (or "HIFI5_PROD_1123_asic_wUPG"), *T*: to "Release", then click :guilabel:`Build Active`. The generated image will be placed under ``project\image``, the name of the image is :file:`dsp_all.bin`.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
By default, the DSP is not enabled. It requires KM4 core to power it on. Enable DSP core in SDK menuconfig:
...
1. Download the DSP firmware and MCU application firmware separately: this method is convenient for development. The image names are shown in the following figure, and the image address refer to the *DSP IMG* Physical address in Flash Layout table.

.. figure:: ../figures/download_the_dsp_and_MCU_application_separately.png
   :scale: 90%
   :align: center

2. Combine DSP firmware and MCU application firmware into one firmware then download together: This method is convenient for OTA upgrade. For details, refer to the of OTA section.


After download process is over, reset the board, and you should see *Hello DSP World* from a UART tool.
...
.. caution::
   When adding project files, be sure to use relative paths. As shown in the figure above\:\ ``Create link locations relative to: PROJECT_LOC``.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Co-processor, O3 and SIMD compilation options can greatly improve the utilization of DSP hardware resources. But these options are not available for all codes. With -O3 option, the compiler may significantly adjust the order of execution. This option is generally safe, but there is a small chance of problems. Some RTOS source files and other ISR handlers “cannot” use co-processor and SIMD vector options. Such as: *-LNO:simd* and *-mcoproc*.
...
3. Note that in this case, the contents of the *.command* and *.ipc_table* sections cannot be loaded into the image because they are not explicitly called. In the link script :file:`RTK_LSP\ldscripts\elf32xtensa.x`, add *KEEP* attribute to these two segments. Note that this linker script is overwritten after each modification of the LSP. The script needs to be reworked.
...
1. Replace your :file:`topology.xml` under the path ``C:\Program Files (x86)\Tensilica\Xtensa OCD Daemon 14.08`` (for Linux, the file path is: :file:`/opt/Tensilica/xocd-14.08/topology.xml`) with the flowing code. Change the usbser value according to your own J-Link S/N.
...
.. note::
   Some warning messages are expected and can be ignored. If you receive an error initializing the XDM driver, you may need to initialize and start the DSP core before debugging.



Under ``<dsp_sdk>\project (or auto_ws)\project_dsp\bin\HIFI5_PROD_1123_asic_UPG\Release``, there is a project_dsp file.
...
Reference Document
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
- For the specific ISS simulation method, refer to the Xtensa document in the Xplorer installation directory: \ *inst_set_sim_ug.pdf*\ .
