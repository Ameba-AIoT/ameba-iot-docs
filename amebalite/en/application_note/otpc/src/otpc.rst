...
Logical Zone
~~~~~~~~~~~~~
You can read and write the logical zone by commands in the following list.
...
In the massive production (MP) stage, another command to program logical zone is **iwpriv**, which has been integrated into RF calibration tools.
This command is only recommended to be used to program Wi-Fi calibration zone.

Physical Zone
~~~~~~~~~~~~~~~
You can read and write the physical zone by commands in the following table. The value of physical zone can only be written from *1* to *0*, please program it carefully.
...
Logical Zone
~~~~~~~~~~~~~~
The OTP can only be programmed once, however some data needs to be overwritten in some reasons.
Therefore, the logical data can be overwritten after format conversion defined by Realtek.
...
System Data
^^^^^^^^^^^^^^^^^^^^^^
The logical area 0x000 ~ 0x01F (32 bytes) will be auto-loaded to system registers by hardware when the system boots.
If the system data hasn't been programmed, system registers will keep initial value that are 0x00, however read logical map will get 0xFF.
...
2. Assume the data is 0x12A03456 in logical address 0x00~0x03 in :ref:`Step 1 <otpc_system_data_example_1_step_1>`. Let 0xA0 make ``OR`` operation with programmed bit[1], and other data keep default value. So the new value to be written is 0x12A23456.
...
2. Assume the data is 0xFFFFFFFF in logical address 0x08~0x0B in :ref:`Step 1 <otpc_system_data_example_2_step_1>`. Let 0x00 make 'OR' operation with programmed bit[0], and other data keep default value. So the new value to be written is 0x00000001.
...
^^^^^^^^^^^^^^^^^^^^^^^
For detailed information about Wi-Fi calibration data, refer to the MP flow document.

Programming Scenarios
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Usually, system data have their initial value, and you can program specific bits according to your demands.
The following table lists some scenarios that specific bits need to be programmed at your requirements.
...
Key Area
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Contents in key area are listed in the following table. For more detailed usage about the keys, refer to the corresponding chapters.
...
Configuration Area
^^^^^^^^^^^^^^^^^^^^^^
Contents in configuration area are listed in the following table. About field's usage in this area, you can get detailed information in the corresponding chapters.
...
.. only:: RTL8726EA
   ...
   .. table:: 
      ...
      +--------+--------+--------------------------------+---------------------------------------------------------------+---------------------------------------------------------------------------------------------+
      | 0x370  | [31:0] | CRC0[31:0]                     | CRC check                                                     | :ref:`CRC <otpc_crc>`                                                                       |
      +--------+--------+--------------------------------+                                                               |                                                                                             |
...
2. Get the valid CRC value without actually enable CRC using the following function:
...
In this area, two keys have their own read protection and write protection. These two keys will be auto-loaded to the hardware.
...
Contents in hidden physical area and usage are listed in the following table.
