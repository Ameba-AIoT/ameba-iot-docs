...
The method is to modify the address of **IMG_BOOT_OTA2** in ``component\soc\amebalite\usrcfg\ameba_flashcfg.c`` . After burning the Bootloader OTA2 into Flash through OTA, Boot ROM will decide whether to use Bootloader OTA1 or Bootloader OTA2 according to the version number.
...
If the anti-rollback function is enabled to ensure that the version of the Bootloader can only be incrementing but cannot be rolled back, the version of the Bootloader needs to be changed before compiling. That is, **IMG_VER** in ``{SDK}\amebalite_gcc_project\manifest.json`` needs to be modified.
...
----------------------------------------
Before loading APP IMG, the BootLoader will read the Status Register from Flash. If only `Quad Enable (QE)` Bit is set in the output of bitwise AND between Status Register of flash and `status_mask` in Flash_AVL (``{SDK}\component\soc\amebalite\usrcfg\ameba_flashcfg.c``), do nothing, or the output of bitwise AND will be written to the Flash Status Register.
...
.. figure:: ../figures/flash_protect_enable.png
   :scale: 70%
   :align: center
