...
.. table::
   ...
   +-------------------------------+----------------------------------------+-------------------------------------------------------------------------------------------------------------------+
   | Dst auto reload               | PGDMA_InitTypeDef->GDMA_ReloadSrc = 0; | For multi block transfers, the DAR register can be auto-reloaded from its initial value at the end of each block, |
   |                               |                                        |                                                                                                                   |
   |                               | PGDMA_InitTypeDef->GDMA_ReloadDst = 1; | and the SRC address is contiguous.                                                                                |
   +-------------------------------+----------------------------------------+-------------------------------------------------------------------------------------------------------------------+
   | Src & Dst auto reload         | PGDMA_InitTypeDef->GDMA_ReloadSrc = 1; | For multi block transfers, the SAR and DAR register can be auto-reloaded from its initial value at the end of     |
   |                               |                                        |                                                                                                                   |
...
.. table::
   ...
   +--------------------------+-----------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------+
   | Src: Auto-reloading      | PGDMA_InitTypeDef->GDMA_ReloadSrc = 1;  | In source, SAR register can be auto-reloaded from the initial value at the end of each block,                                       |
   |                          |                                         |                                                                                                                                     |
   | Dst: Link list           | PGDMA_InitTypeDef->GDMA_SrcAddr = pSrc; | as shown in :ref:`mbd_source_auto_link_dest`                                                                                        |
   |                          |                                         |                                                                                                                                     |
   |                          | PGDMA_InitTypeDef->GDMA_LlpDstEn = 1;   |                                                                                                                                     |
   +--------------------------+-----------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------+
   | Src: Link list           | PGDMA_InitTypeDef->GDMA_LlpSrcEn = 1;   | Source memory is organized in the form of a linked list, and destination memory is a continuous data block,                         |
   |                          |                                         |                                                                                                                                     |
   | Dst: Continue address    | PGDMA_InitTypeDef->GDMA_DstAddr = pDst; | as shown in :ref:`mbd_link_source_cont_dest`.                                                                                       |
   +--------------------------+-----------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------+
...
.. table:: 
   ...
   |           |                                                                                                       |
   |           | 5. Do :func:`DCache_Invalidate()` in Rx Done Handler to clean the old data, to avoid the problem      |
   |           |                                                                                                       |
   |           |    that the data in the cache is inconsistent with the data in the memory after Rx done if the CPU    |
   |           |                                                                                                       |
